...
def read_customer_and_car_data():
    ...
    customers_data = read_data('Customer Detail.txt')
    cars_data = read_data('Car Detail.txt')
    
    if customers_data is None or cars_data is None:
        return False
    return True
...
def process_credit_card_payment(customer_id):
    try:
        display_invoice(customer_id, "Credit Card")
        while True:
...
def process_e_wallet_payment(customer_id):
    try:
        display_invoice(customer_id, "eWallet")
        print(f"\nSelected: eWallet")
...
def process_bank_info(bank_name, customer_id):
    try:
        display_invoice(customer_id, "Bank Transfer - " + bank_name)
        print(f"\nSelected: {bank_name}")
        ...
        confirm_payment("Bank Transfer - " + bank_name, customer_id)
    except Exception as e:
...
def confirm_payment(wallet_name, customer_id):
    try:
        ...
        clear_screen()  # Clear the screen before displaying success message
        print("Payment confirmed!")
        display_invoice(customer_id, wallet_name)
    except Exception as e:
...
def cash_payment(customer_id):
    try:
        ...
        display_invoice(customer_id, "Cash")

        customer, _ = display_customer_and_car(customer_id)
        if customer:
...
def display_invoice(customer_id, payment_method):
    try:
...
def process_bank_transfer_payment(customer_id):
    try:
        ...
        while True:
            ...
            if choice == "1":
                process_bank_info("MayBank", customer_id)
                break
            elif choice == "2":
                process_bank_info("Public Bank", customer_id)
                break
            elif choice == "3":
                process_bank_info("Bank Islam", customer_id)
                break
            elif choice == "4":
                main()
                break
        ...
        if read_customer_and_car_data():
            ...
            while True:
                ...
                if choice == "1":
                    ...
                    process_credit_card_payment(customer_id)
                    break
                elif choice == "2":
                    print("\nSelected: eWallet")
                    process_e_wallet_payment(customer_id)
                    break
                elif choice == "3":
                    print("\nSelected: Bank Transfer")
                    process_bank_transfer_payment(customer_id)
                    break
                elif choice == "4":
                    print("\nSelected: Cash")
